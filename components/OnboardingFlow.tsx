          ...
          {!authDone ? (
            <>
              <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
                {[{ label: 'Continue with Apple', bg: '#0F2A4A', color: '#fff', icon: '' }, { label: 'Continue with Google', bg: '#fff', color: '#334155', icon: 'G', border: '1.5px solid #E2E8F0' }].map(s => (
                  <button key={s.label} onClick={() => setAuthDone(true)} style={{ flex: 1, padding: '12px 8px', borderRadius: 12, border: (s as any).border || 'none', background: s.bg, color: s.color, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          {selGoal && (
            <div style={{ animation: 'obFadeUp 0.25s ease', display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ position: 'relative' }}>
                ...
                <input type="number" value={goalAmt} onChange={e => setGoalAmt(e.target.value)} placeholder="Target amount" style={{ width: '100%', padding: '13px 14px 13px 28px', borderRadius: 12, border: '1.5px solid #E2E8F0', fontSize: 16, fontWeight: 700, color: '#0F2A4A', fontFamily: "'DM Mono',monospace", outline: 'none', background: '#fff' }} />
              </div>
              <input value={goalDate} onChange={e => setGoalDate(e.target.value)} placeholder="Target date (e.g. Aug 2027)" style={{ width: '100%', padding: '13px 14px', borderRadius: 12, border: '1.5px solid #E2E8F0', fontSize: 14, color: '#0F2A4A', fontFamily: 'inherit', outline: 'none', background: '#fff' }} />
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '0 28px' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', paddingTop: 4 }}>
          ...
          {[
            ...
            const isLinked = linked.includes(a.id);
            const isLinking = linking === a.id;
            return (
              <div key={a.id} onClick={() => { if (!isLinked && !isLinking) linkAccount(a.id); }} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '14px 16px', borderRadius: 16, border: '2px solid ' + (isLinked ? '#15803D' : '#E2E8F0'), background: isLinked ? '#F0FDF4' : '#fff', marginBottom: 10, cursor: isLinked ? 'default' : 'pointer', transition: 'all 0.2s' }}>
                <span style={{ fontSize: 26 }}>{a.icon}</span>
                <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 700, color: '#0F2A4A' }}>{a.label}</div><div style={{ fontSize: 12, color: '#64748B', marginTop: 1 }}>{a.sub}</div></div>
